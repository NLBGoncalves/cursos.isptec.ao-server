...
exports.registrationUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isEmailExist) {
            return next(new ErrorHandler_1.default("Email já existe", 400));
        }
        const user = {
            name,
            email,
...
exports.activateUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (newUser.activationCode !== activation_code) {
            return next(new ErrorHandler_1.default("Código de ativação inválido", 400));
        }
...
exports.loginUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    ...
    try {
        ...
        if (!email || !password) {
            return next(new ErrorHandler_1.default("Por favor insira e-mail e senha", 400));
        }
        const user = await user_model_1.default.findOne({ email }).select("+password");
        if (!user) {
            return next(new ErrorHandler_1.default("E-mail ou senha inválida", 400));
        }
...
exports.updateAccessToken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!session) {
            return next(new ErrorHandler_1.default("Faça login para acessar esses recursos!", 400));
        }
...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!oldPassword || !newPassword) {
            return next(new ErrorHandler_1.default("Por favor insira a senha antiga e a nova", 400));
        }
        const user = await user_model_1.default.findById(req.user?._id).select("+password");
        if (user?.password === undefined) {
            return next(new ErrorHandler_1.default("Usuário Inválido", 400));
        }
        const isPasswordMatch = await user?.comparePassword(oldPassword);
        if (!isPasswordMatch) {
            return next(new ErrorHandler_1.default("Senha antiga inválida", 400));
        }
...
exports.deleteUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!user) {
            return next(new ErrorHandler_1.default("Usuário não encontrado", 404));
        }

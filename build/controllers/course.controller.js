...
exports.getCourseByUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!courseExists) {
            return next(new ErrorHandler_1.default("Você não está qualificado para acessar este curso", 404));
        }
...
exports.addQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(contentId)) {
            return next(new ErrorHandler_1.default("ID de conteúdo inválido", 400));
        }
        ...
        await notification_Model_1.default.create({
            ...
            message: `Você tem uma nova pergunta em ${couseContent.title}`,
        });
    ...
    try {
        ...
        if (req.user?._id === question.user._id) {
            ...
            await notification_Model_1.default.create({
                ...
                message: `Você tem uma nova resposta para pergunta em ${couseContent.title}`,
            });
        }
        else {
            const data = {
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await notification_Model_1.default.create({
            ...
            message: `${req.user?.name} fez uma revisão em ${course?.name}`,
        });
        res.status(200).json({
            success: true,
            course,
...
exports.addReplyToReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const course = await course_model_1.default.findById(courseId);
        if (!course) {
            return next(new ErrorHandler_1.default("Curso não encontrado", 404));
        }
        const review = course?.reviews?.find((rev) => rev._id.toString() === reviewId);
        if (!review) {
            return next(new ErrorHandler_1.default("Comentário não encontrado", 404));
        }
        const replyData = {
            user: req.user,
            comment,

...
export const loginUser = CatchAsyncError(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!email || !password) {
        return next(new ErrorHandler("Por favor insira e-mail e senha", 400));
      }

      const user = await userModel.findOne({ email }).select("+password");

      if (!user) {
        return next(new ErrorHandler("E-mail ou senha inválida", 400));
      }
...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!session) {
        return next(
          new ErrorHandler("Faça login para acessar esses recursos!", 400)
        );
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!oldPassword || !newPassword) {
        return next(new ErrorHandler("Por favor insira a senha antiga e a nova", 400));
      }
